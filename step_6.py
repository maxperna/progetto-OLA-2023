...
for e in range(0, n_experiments_S6):
    ...
    # ucb1_learner = UCB1_Learner_old(n_arms_pricing, Collector._max_price)
    ...
    # exp3_learner = EXP3_Learner(n_arms_pricing, gamma)
    exp3_learner = EXP3_Learner(
        n_arms=n_arms_pricing,
        gamma=gamma,
        clicks=n_clicks,
    ...
    for t in range(0, T):
        ...
        # cd_ucb1_learner.update(
        #     pulled_arm, reward, Collector.prices[pulled_arm], opt_vec[t], n_arms_pricing
        # )
...
# regret_ucb1 = opt_vec - ucb1_rewards_per_experiment  # row = exp, col = t
...
# plt.plot(cum_avg_regret_sw_ucb1, 'o')
...
plt.legend(["UCB1", "EXP3"])  # "SW_UCB1", "CUSUM_UCB1",
plt.title("Cumulative Regret")
fig = plt.gcf()
plt.show()
...
# plt.plot(cum_avg_reward_sw_ucb1, 'o')
